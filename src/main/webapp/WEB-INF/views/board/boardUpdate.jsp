...
<form id="updateForm">
	<div class="card shadow mb-4">
		<div class="card-header py-3">
			<div class="row">
				<div class="col-sm-12 col-md-6">
					<div class="flex-row-baseline">
						<select id="boardCat" name="boardCat" class="custom-select custom-select-sm form-control form-control-sm" style="width: 70px; margin-right: 15px;">
	        ...
	        <div id="fileUploadedListBody"></div>
	        <div id="fileListBody"></div>
				</div>
				<div class="col-sm-12 col-md-6">
					<div class="fr">
						<input id="boardUpdateBtn" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" type="button" value="수정완료">
					</div>
				</div>
			</div>
		</div>
	...
	<input name="boardDiv" type="hidden" />
	<input name="atchId" type="hidden" />
	<input name="delSeq" type="hidden" />
</form>
...
<script>
	...
	$(function() {
		$.ajax({
	        url: '${CPATH}/api/v1/boards/${param.no}'
	      , dataType: 'json'
	      , success: function(data) {
	    	  var board = data;

	        $('[name="boardCat"]').val(board.boardCat);
	        $('[name="boardTtl"]').val(board.boardTtl);
	        $('[name="boardCont"]').text(board.boardCont);
	        $('input[name="boardNo"]').val(board.boardNo);
	        $('input[name="boardDiv"]').val(board.boardDiv);
	        $('input[name="atchId"]').val(board.atchId);

	        var atchList = board.atchList;
	        var fileUploadedListBody = $('#fileUploadedListBody');
	        if(atchList != null && atchList.length > 0) {
	        	var html = null;
	        	var nameTag = null;
	        	var sizeTag = null;
	        	var xBtnTag = null;

		        $.each(atchList, function(i, atch) {
		        	html = $('<div>');

		         	nameTag = $('<span>');
		         	nameTag.append($('<input class="atchSeq" type="hidden">').attr('value', atch.atchSeq));
		         	atchName = atch.atchOrigNm;
		         	if(atch.atchExtn != "" && atch.atchExtn != null) {
		         		atchName = atchName + "." + atch.atchExtn;
		         	}
		         	nameTag.append($('<span style="color: #224ACC; margin-right: 5px;">').text(atchName));

		         	html.append(nameTag);

		         	sizeTag = $('<span>').text(atch.cnvrtSize);
		         	html.append(sizeTag);

		         	xBtnTag = '<span class="x-btn" style="cursor: pointer;">'
		         	...
		         	fileUploadedListBody.append(html);
		        });
	        }
	      }
	  })
	...
	$(document).on('click', '.x-btn', function () {
		var atchSeq = $(this).parent().find('.atchSeq').val();
		delSeq.push(atchSeq);
		$(this).parent().remove();
	});
